...
if (walkableareas.length > 0) { 
    ...
    <div style="padding: 8px; background: #2d2d30; margin-bottom: 6px; border-radius: 3px; 
    font-size: 11px; display: flex; justify-content: space-between; align-items: center;"
    </div>
    <button onclick= "deletewalkablearea(${index})" style="padding: 4px
    8px; font-size: 10px; background: #d13438; color: white; border: none; border-radius: 2px; cursor:
    ...
    function getwalkablearesforouput() {
    return walkableareas.map(area => area.map(p => Array.isArray(p) ? [p[0], p[1]] : [p.x, p.y])));
}
