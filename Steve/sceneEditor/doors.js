...
function finishDoorPolygon(points) {
	...
	if (!nextScene) {
		alert('Please select a next scene');
		return false;
	}
	...
	if (doors.length > 0) {
		...
		doorsList.innerHTML = doors.map((door, index) => `
			<div style="padding: 8px; background: #2d2d30; margin-bottom: 6px; border-radius: 3px; font-size: 11px; display: flex; justify-content: space-between; align-items: center;">
				...
				<button onclick="deleteDoor(${index})" style="padding: 4px 8px; font-size: 10px; background: #d13438; color: white; border: none; border-radius: 2px; cursor: pointer;">Delete</button>
			</div>
		`).join('');
	} else {
